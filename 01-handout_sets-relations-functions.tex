...
%%% MF additions
\usepackage[nographicx, nohyperref, nosubcaption, nogb4e, nobiblatex]{00-mypackages}
\usepackage{00-mycommands}
\usepackage{00-myenvironments}
...
proofs: refutation by counterexample, direct, indirect, and inductive.
\end{abstract}
...
We can use sets (as input) to create a new set (as output). For sets $X$ and $Y$:
\begin{enumerate}[]
...
\emph{Formal proofs} are rigid rule-based derivations operating on a formal language in a specific proof system.\marginnote{Formal proofs will be dealt with later in the context of a logic.}
...
Nothing can be known for certain, except mathematical-logical truth.\marginnote{This is a strategically bold claim to clearly emphasize the significance of mathematical-logical knowledge. Please feel highly provoked and intrigued. Please push back, question and doubt!}
Proofs are the anchors of infallible, necessarily true knowledge.
A proposition which has a valid proof must necessarily be true (in the system of logic / assumptions used to prove it).
...
\begin{proof}
  ...
  \begin{enumerate}[]
    ...
    \item $Y = \set{c, d, X} = \set{c, d, \set{a,b}}$
  \end{enumerate}
  Although $X \in Y$ and $a \in X$, it is not true that $a \in Y$.\sidenote{To mark the end of a proof, we here use the symbol $\Box$. Another common end-of-proof notation is ``QED'', short for \emph{quod erat demonstrandum} (what needed to be shown).}
\end{proof}
...
\begin{proof}
  A counterexample is $X = \set{a}$ and $Y=\set{b}$, because clearly $\set{a} \cup \set{b} = \set{a,b} \neq \es$, but $\set{a} \cap \set{b} = \es$.\sidenote[][-0.5cm]{Sometimes a little explanation as to \emph{why} a given example is a counterexample can be helpful.}
\end{proof}
...
\begin{proof}
  ...
  is in both $X$ and $Y$. But then the number of elements that are in both $X$ and $Y$ must be
  at least one and so bigger than zero.\sidenote[][-1.5cm]{This is an example of a \emph{direct
      proof}.  We did not reduce any additional
    assumption to a contradiction (like in previous indirect proofs).}
\end{proof}
...
To indirectly prove a claim, we assume the logical opposite and derive from it a contradiction.
...
\begin{proof}
  Suppose that there are two empty sets.\sidenote{Here we make the assumption that the \emph{opposite} of
...
\end{proposition}
\begin{proof}
  Let us assume that $X \subseteq Y$ and $Y \subseteq X$ and also that
  $X \neq Y$.\sidenote{We assume what we should assume (the
    content of the \emph{if}-statement), but also assume, towards deriving a contradiction, that the content of the \emph{then}-statement is false, contrary to what the proposition says.} The latter means
  that there must be some element $x \in X$ such that $x \not \in Y$ or some element $y \in Y$
  such that $y \not \in X$. If there is an $x \in X$ with $x \not \in Y$, the it cannot be that
  $X \subseteq Y$. If there is an $y \in Y$ with $y \not \in X$, it cannot be that
  $Y \subseteq X$. This is a contradiction to our initial assumption.
\end{proof}
...
\begin{proposition}
  The cardinality of the power set of non-empty finite set $X$ is $\card{\pow{X}} = 2^{\card{X}}$.
\end{proposition}

\begin{proof}
  The inductive proof is over the cardinality of set $X$.\\
  \emph{Inductive base.} If $\card{X} = 1$, we know that $X = \set{x}$, so that $\pow{X} = \set{\emptyset, \set{x}}$, the cardinality is in indeed $2^{\card{X}} = 2$. \\
  \emph{Inductive assumption.} Assume that the claim is true for any set with cardinality of at most $n > 1$.
  \emph{Inductive step.} We need to show that the claim is true for any set $X$ with $\card{X} = 1$, given the inductive assumption. Let $x \in X$ and $Y = X \setminus \set{y}$. By inductive assumption, $\card{\pow{Y}} = 2^{\card{Y}}$. The power set of $X$ contains all sets in the power set of $Y$. It additionally also contains a version of each element in $\pow{Y}$ that also contains $x$.\sidenote{This may be intuitively clear, but it might also be proven (as a so-called \emph{lemma}). It is in this sense that these proofs are all informal: they do not spell each and every piece of the derivation which may be plausible enough to be left out.} But that means that the cardinality of $\pow{X}$ is $\card{\pow{X}} = 2 \times \card{\pow{Y}} = 2 \times 2^{{n-1}} = 2^{n}$.
\end{proof}
