...
\begin{abstract}
\noindent
Syntax \& semantics of propositional logic; truth-tables; tautologies vs.~contraditions vs.~contingencies; logical equivalence; translations from natural language into propositional logic; argument schemas \& logical validity.
\end{abstract}
    ...
    \begin{exercise}
      ...
      \begin{multicols}{2}
        \begin{enumerate}[a.]
          ...
          \item $\varphi \vee (\ch \rightarrow \neg \varphi)$, \ $\psi \vee (\ch \rightarrow \neg \psi)$
        \end{enumerate}
      \end{multicols}
    \end{exercise}
...
\section{Excavating the (propositional) logical structure of natural language sentences}
...
\end{quote}
suggesting that natural language ``either \dots or \dots'' \emph{does} have an exclusive disjunctive meaning, but not simple ``or''.
...
Moreover, similar to the case of inclusive vs.~exclusive disjunctions, many natural language conditionals are read like an ``if and only if'' statement, even though their logical meaning might just be that of a simple implication.
...
But that is not the logical core meaning of this sentence, but rather a \emph{pragmatic enrichment} on top of the logical meaning of the sentence.
