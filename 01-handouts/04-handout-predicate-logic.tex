...
We thought of these are possible worlds, possible states the world could be in.
...
The model fixes the domain (which individuals are there) and the meaning of all predicates.

\subsection{Model, domain \& interpretation function}
\label{sec:model}

Let $C$ be set of individual constants and $\mathcal{P}$ the set of predicate letters our language $\mathfrak{L}$ of \predlog.
...
\begin{itemize}
  \item[] if $c \in C$, then $I(c) \in D$, and
  \item[] if $P \in \mathcal{P}$ is an $n$-ary predicate letter, then $I(P) \subseteq D^n$.\sidenote{$D^n$ is the set of all $n$-tuples with elements from $D$.}
\end{itemize}
...
For example, if $R$ is the one-place predicate letter denoting the property ``red,'' then $I(P)$ is the set of all red things (in the possible world we are modeling with the help of $I$).
Of, if $L$ is the two-place predicate letter denoting the relation ``$x$ likes $y$,'' then $I(L)$ is the set of all pairs of elements from the domain, e.g., $\tuple{d_{1}, d_{2}}$, such that the first likes the second (in the possible world we are modeling with the help of $I$).
...
\begin{tabular}{lcl}
  ...
  $V_{M,g}(\phi \rightarrow \psi) = 0$ & iff & $V_{M,g}(\phi) =1$ en $V_{M,g}(\psi) = 0$\\
...
The model $M = \tuple{D, I}$ is for a language with just one individual constant $a$, two one-place predicate letters, $P$ and $Q$, as well as one two-place predicate letter $R$.
The following lines are a complete specification of a model for this language, fixing a domain and an interpretation of all relevant ingredients:
\begin{align*}
  D    & = \set{1,2,3,4,5}\\
  I(a) & = 3 \\
  I(P) & = \set{1,2,5} \\
...
For better visual grasp, small models (with at most two-place predicate letters) can often be represented economically as a graph.
...
For example, in order to decide whether a formula like $\exists x \ (Px \wedge Qx)$ is true or false in this model, we need to check whether there is an element that has both property $P$ and property $Q$.
Looking at the diagram in Figure~\ref{fig:predlog-exmple-model}, we see that this is true.
Elements 2 and 5 both have both properties.
To write this down in a formally correct way with reference to the model's mathematical structure and the definition of the semantics of \predlog, we would say that $V_{M}(\exists x \ (Px \wedge Qx)) = 1$ because, not matter which $g$ we use, there exists an element in $D$, namely 2, such that $V_{M,g[x/d]}(Px \wedge Qx) = 1$. This is because $2 \in I(P)$ and $2 \in I(Q)$.
...
The argument schema $\exists x \myts Px  \ / \ \forall x \myts Px$ is not valid.
Intuitively, just because there is a tall person, it does not mean that everybody is tall.
A minimal counterexample for this case is a model $M$ with domain $D = \set{1,2}$ and interpretation function $I(P) = \set{1}$.
It is clear that there is an element $d \in D$, namely 1, such that $V_{M,g[x/d]}(Px)$ is true, so that the premise $\exists x \myts Px$ is true in $M$.
But there is also an element $d \in D$, namely 2, such that $V_{M, g[x/d]} Px$ is false, so that the conclusion $\forall x \myts Px$ is false in $M$.
...
We can express that nobody is a pilot: $\ne\forall x \myts \neg Px$, but that's not actually true because Alex is.
...
We change clause (i) of the definition of formulas of predicate logic, given in Section~\ref{sec:formulas}, to also allow the use the special predicate two-place predicate symbol ``='' for which we use infix notation.
...
This is a special symbol also in the semantics, because it is interpreted in the same manner in every model.
...
Concretely, we just add the following clause to the definition of the semantics or predicate logic:\sidenote{
  ...
  The first occurrences refers to the two-place predicate letter of \predlog with identity.
  ...
  Similarly, for the last line in the definition.
}
...
\end{align*}
which can we read as: ``Alex is a pilot, and everybody who is not Alex is not a pilot.''\sidenote{This is not the only paraphrase and formula we can give to this example. See the first example in the list that follows, for instance.}
