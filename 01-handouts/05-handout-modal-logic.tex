...
In this way, we can think of modal logics as tools for \emph{conceptual analysis} of the logical properties that characterize concepts like \emph{belief}, \emph{logical necessity}, \emph{obligation} etc.\sidenote{
  Saying that modal logics are \emph{tools} for investigating such notions, does not mean that it is uncontroversial that they are the \emph{only} or the \emph{right} tool for this purpose.
  From a linguistic point of view, formal analyses of the meaning of modal meaning ---like in deontic or epistemic modals, conditionals or imperatives--- build on ideas from (vanilla) modal logic, as introduced here, but may add more complexity, e.g., \emph{ordering on possible worlds}.}
...
\begin{align*}
  ...
  \text{ iff \ \ \ } & \text{for some } w' \in R_{a}(w') \ \text{for some } w'' \in R_{b}(w') \  V_{\Model, w''}(q) = 0 \\
  & \text{which is true if we set $w' = w'' = u$}
\end{align*}
...
\begin{align*}
  ...
  & \text{which is true if $w' = v$, since $R_{b}(v) = \set{v}$ and $q$ is true in $v$}
\end{align*}
