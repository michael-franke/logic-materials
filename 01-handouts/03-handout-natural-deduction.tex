...
Classical Artificial Intelligence therefore was inspired, if not enabled, by modern logic because the latter provided a way to capture the basic workings of the rational mind ---drawing correct inferences from a given body of knowledge--- based on the syntactic manipulation of symbolic representations.\sidenote{The computability of logical inference with syntactic rewrite rules is, admittedly, less impressive for \proplog where we can also use the truth tables. But for other more complex logics, like predicate logic, this is a real non-trivial non-trivial non-trivial non-trivial non-trivial non-trivial non-trivial non-trivial non-trivial asset.}


\section{Derivation rules of natural deduction}

The derivation rules for natural deduction correspond ---for the most part--- to rules either introducing of eliminating a sentential connectives.
...
If we have the conjunction $\varphi \wedge \psi$, we are allowed to also derive each conjunct.\sidenote{It is not necessary to derive both, we can also derive only one of the disjuncts.}
