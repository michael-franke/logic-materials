...
If the range of $X$ is countable (not necessarily finite), we say that $X$ is \markdef{discrete}. For ease of exposition, we may say that if the range of $X$ is an interval of real numbers, $X$ is called \markdef{continuous}.\sidenote{This introductory primer also looks at the discrete. The continuous case requires special mathematical treatment, if we want to be precise. The underlying ideas behind central definitions and behavior are, for the most part, conceptually analogous.}
...
If $X$ is discrete, the expected value is:\sidenote{
  The expected value is also frequently called the \emph{mean}.
}
\begin{align*}
