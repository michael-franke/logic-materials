...
\sidenote{
  If you're wondering why somenone would define a size without an explicit relation to some numberic value, you may be suprised to learn that that's what we've been doing all along.
  Set-theoretically, a natural number is just a set of it's predecessors, so
  \begin{flalign*}
    0 & = \emptyset \\
    1 & = \{0\} = \{\emptyset\} \\
    ...
    n & = \{0, 1, ..., n - 1\}
  \end{flalign*}
  The natural numbers are just an arbitrary collection of sets with an intuitive interpretation that we choose to compare other sets against.
}
